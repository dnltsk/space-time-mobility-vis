...
function showTraficTooltip_firstLevel(sourcePath, sourcePos){
	...
	var xPosition = d3.mouse(d3.select('body').node())[0]+20;
	var yPosition = d3.mouse(d3.select('body').node())[1] - 100;
	...
	if(trafficDirection == TRAFFIC_DIRECTION_SOURCE){
		//traffic
		d3.keys(wholeTrafficMatrix[srcLocationType][destLocationType][sourcePos]).forEach(function(_destPos){
			if(_destPos!="tts" && _destPos!="ttd" && _destPos!="tt"){
				trafficValue += wholeTrafficMatrix[srcLocationType][destLocationType][sourcePos][_destPos];
				console.log("counting ausgehenden Verkehr: srcLocationType="+srcLocationType+" destLocationType="+destLocationType+
					        "\nquelle = "+sourcePos+" ziel="+_destPos+" value="+wholeTrafficMatrix[srcLocationType][destLocationType][sourcePos][_destPos]+
					        " sum="+trafficValue);
			}
		});
		tt.append("p")
		...
		//traffic
		d3.keys(wholeTrafficMatrix[srcLocationType][destLocationType]).forEach(function(_sourcePos){
			if(_sourcePos!="tts" && _sourcePos!="ttd" && _sourcePos!="tt"){
				if(wholeTrafficMatrix[srcLocationType][destLocationType][_sourcePos][sourcePos]){
					trafficValue += wholeTrafficMatrix[srcLocationType][destLocationType][_sourcePos][sourcePos];
					console.log("counting eingehenden Verkehr: srcLocationType="+srcLocationType+" destLocationType="+destLocationType+
						        "\nquelle = "+_sourcePos+" ziel="+sourcePos+" value="+wholeTrafficMatrix[srcLocationType][destLocationType][_sourcePos][sourcePos]+
						        " sum="+trafficValue);
				}
			}
		});
...
/*
 * Aufschl�sselung der Quell-Standorte!
 * --> von ALL ignorieren!
 * --> und immer zum gew�hlten destLocationType! 
 */
function addVorgaengerPie(sourcePos, svgElement){
	...
	d3.keys(wholeTrafficMatrix).forEach(function(_srcLocationType){
		if(_srcLocationType.indexOf("ALL") == -1
			...
			if(trafficDirection == TRAFFIC_DIRECTION_SOURCE){
				//Quellverkehr!
				if(wholeTrafficMatrix[_srcLocationType][destLocationType][sourcePos]){
					//
					d3.keys(wholeTrafficMatrix[_srcLocationType][destLocationType][sourcePos]).forEach(function(_destPos){
						if(_destPos.indexOf("tt") == -1 
								&& _destPos.indexOf("tts") == -1 
								&& _destPos.indexOf("ttd") == -1){
				...
				//Zielverkehr!
				d3.keys(wholeTrafficMatrix[_srcLocationType][destLocationType]).forEach(function(_sourcePos){
					//
					if(_sourcePos.indexOf("tt") == -1 
							&& _sourcePos.indexOf("tts") == -1 
							&& _sourcePos.indexOf("ttd") == -1){
						if(wholeTrafficMatrix[_srcLocationType][destLocationType][_sourcePos][sourcePos]){
								trafficVolume += wholeTrafficMatrix[_srcLocationType][destLocationType][_sourcePos][sourcePos];
						}
					}
				});
			}
			...
			if(trafficVolume>0){
				console.log("Vorg�nger-Standorte _srcLocationType = "+_srcLocationType+" trafficVolume = "+trafficVolume);
...
/*
 * Aufschl�sselung der Ziel-Standorte!
 * --> zu ALL ignorieren!
 * --> und immer von gew�hlter srcLocationType! 
 */
function addFolgePie(sourcePos, svgElement){
	...
	d3.keys(wholeTrafficMatrix[srcLocationType]).forEach(function(_destLocationType){
		if(_destLocationType.indexOf("ALL") == -1
			...
			if(trafficDirection == TRAFFIC_DIRECTION_SOURCE){
				//Quellverkehr!
				if(wholeTrafficMatrix[srcLocationType][_destLocationType][sourcePos]){
					//
				...
				//Zielverkehr!
				d3.keys(wholeTrafficMatrix[srcLocationType][_destLocationType]).forEach(function(_sourcePos){
					//
					if(_sourcePos.indexOf("tt") == -1 
							&& _sourcePos.indexOf("tts") == -1 
							&& _sourcePos.indexOf("ttd") == -1){
						if(wholeTrafficMatrix[srcLocationType][_destLocationType][_sourcePos][sourcePos]){
								trafficVolume += wholeTrafficMatrix[srcLocationType][_destLocationType][_sourcePos][sourcePos];
						}
					}
				});
			...
			if(trafficVolume>0){
				console.log("Folge-Standorte pie _destLocationType = "+_destLocationType+" trafficVolume = "+trafficVolume);
