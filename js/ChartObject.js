var ChartObject = function(){
	...
	this.initScales = function(){
		...
		this.yScale.domain([0,300000]);
		//this.yScale.domain([
		//	d3.min(thisJson.chartData, function(c) { return d3.min(c.values, function(v) { return v.value*100; }); }),
		//	d3.max(thisJson.chartData, function(c) { return d3.max(c.values, function(v) { return v.value*100; }); })
		//]);
	...
	this.draw = function(completeWidth, completeHeight){
		...
		var line = d3.svg.line()
			...
			.y(function(d) { return thisJson.yScale(d.value*96.5); });
		...
		if(!thisJson.svgGroup.selectAll("g.title").empty()){
			...
			thisJson.svgGroup.append("g")
					...
					.text("Genutzte Standorte nach Tageszeit");
		}
	};
