...
function animationPlay(){
	...
	ANIMATION_TRIGGER.transition()
					 ...
					 .attr("value",1)
					 .each("end", function(){
						 window.setTimeout(2000, animationPause);
					 });
	startMapAnimation();
	startNetAnimation();
	...
	control.title="Animation pausieren";
...
function animationPause(){
	...
	var control = document.getElementById("animationPlayControl");
	control.src="img/stop_32x32.png";
	control.title="Animation stoppen (Karte neu laden)";
...
function initLoadActivities(nextStep){
	...
	d3.json("json/animation/possibilities.json", 
			function(d){
				animationData = d.possibilityChains;
				...
				MAP_SVG.selectAll("g#dots").remove();
...
function appendDot(chain, i){
	DOTS_SVG.selectAll("#chain"+i)
		...
		.style("fill",
				function(d){
			return LEGEND_DATA.colorMap.layer_possibilities[d[0].activity_type]
		})
		.style("fill-opacity", 
				function(d){ 
			if(d[0].from == 0){
...
function nextMapChainLink(dotThis){
	...
	dot.style("fill",
			  function(d){
					return LEGEND_DATA.colorMap.layer_possibilities[d[d.currentIndex].activity_type];
	   })
	   .style("fill-opacity", 0)
	   .attr("r", 3)
	...
	dot.transition()
		...
		.each("end", function(d){
			...
			if(d[d.currentIndex].to*40 - currentTime2 < 500){
				d3.select(this)
					.transition()
					.delay(function(d){return d[d.currentIndex].to*40 - currentTime2})
					.duration(0)
					.ease("linear")
					.style("fill-opacity",0)
					.each("end", function(){
...
function initNetChart(){
	...
	svg.append("g")
		...
		.text("Bewegungen zwischen Standorten");
	...
	d3.json("json/animation/net.json", function(error, json) {
	  ...
	  var link = netContent.append("svg:g").selectAll("path")
		  ...
		  .style("stroke", function(d){ 
		  	  return LEGEND_DATA.colorMap.layer_possibilities[json.nodes[d.target].name]; 
		  })
		  .style("fill","none");
	  ...
	  var node = netContent.selectAll("circle")
	      ...
	      .attr("r", function(d){
	    	  return Math.sqrt(d.frames[0].contains *1.5 / Math.PI); 
	      })
